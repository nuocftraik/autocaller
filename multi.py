import requests                              # Thư viện requests để gửi HTTP requests (RESTful APIs).
import time                                  # Thư viện time để xử lý delay, sleep giữa các request.
import urllib3                               # Thư viện urllib3 để quản lý kết nối HTTP thấp hơn requests.
import random                                # Thư viện random để sinh số hoặc ký tự ngẫu nhiên.
import string                                # Thư viện string để lấy danh sách ký tự chữ cái, số.
import concurrent.futures                    # Dùng để tạo và quản lý đa luồng (ThreadPoolExecutor).
from colorama import Fore, Style, init       # Thư viện để in màu chữ trên terminal (đẹp mắt).
import sys                                   # Thư viện xử lý dòng lệnh (sys.argv, sys.exit,...)

init(autoreset=True)                         # Khởi tạo colorama và tự động reset lại style sau mỗi dòng in.

# Tắt cảnh báo "InsecureRequestWarning" nếu gửi request không có chứng chỉ SSL hợp lệ.
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# Tạo HTTP connection pool với yêu cầu có chứng chỉ xác thực (dù thực tế không dùng sau đó).
http = urllib3.PoolManager(
    cert_reqs='CERT_REQUIRED',                       # Yêu cầu chứng chỉ hợp lệ.
    ca_certs='/path/to/your/certificate-authority-bundle-file'  # Chỉ định CA certificate (không dùng).
)

# --- Đọc tham số từ command line ---
if len(sys.argv) < 3:
    print("Cú pháp đúng: python yourfile.py <count> <sdt1> <sdt2> ...")
    sys.exit()

count = int(sys.argv[1])      # số lần lặp
sdt_list = sys.argv[2:]       # danh sách số điện thoại

# --- Hàm chuẩn hoá số điện thoại ---
def sdtt(sdt):
    if sdt.startswith("0"):
        return "+84" + sdt[1:]
    return sdt                        # Gán số điện thoại đã chuyển đổi

# Hàm tạo một email ngẫu nhiên với tên ngẫu nhiên và domain mặc định là example.com
def generate_random_email(domain='example.com'):
    length = random.randint(5, 10)                   # Độ dài tên email là 5 đến 10 ký tự
    email_name = ''.join(random.choices(string.ascii_lowercase + string.digits, k=length))  # Sinh chuỗi ngẫu nhiên
    email = f'{email_name}@{domain}'                 # Gộp thành địa chỉ email
    return email

random_email = generate_random_email()               # Tạo email ngẫu nhiên (dù không được sử dụng sau đó)
...
functions = [                                               # Danh sách hàm cần chạy
...
# --- Vòng lặp chính chạy đa luồng ---
with concurrent.futures.ThreadPoolExecutor() as executor:
    for i in range(count):  # Lặp n lần
        print(Fore.MAGENTA + f"\n===== VÒNG LẶP {i+1}/{count} =====" + Style.RESET_ALL)

        for sdt in sdt_list:               # chạy với từng số
            sdt_chuyen_doi = sdtt(sdt)
            globals()['sdt'] = sdt_chuyen_doi

            print(Fore.YELLOW + f"=== ĐANG CHẠY CHO SỐ: {sdt_chuyen_doi} ===" + Style.RESET_ALL)

            for func in functions:
                print(Fore.CYAN + f"  → Gọi {func.__name__} cho {sdt_chuyen_doi}" + Style.RESET_ALL)
                executor.submit(func)  # gọi API
                time.sleep(1)
